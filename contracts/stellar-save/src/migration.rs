...
pub fn migrate(env: &Env) -> Result<(), StellarSaveError> {
    ...
    env.storage()
        .persistent()
        .set(&version_key, &STORAGE_VERSION);
...
fn migrate_v1_to_v2(env: &Env) -> Result<(), StellarSaveError> {
    ...
    let total_groups: u64 = env
        .storage()
        .persistent()
        .get(&total_groups_key)
        .unwrap_or(0);
...
mod tests {
    ...
    fn test_migrate_v1_to_v2_empty_contract() {
        ...
        assert_eq!(
            env.storage().persistent().get::<bool>(&pause_key).unwrap(),
            false
        );
        assert_eq!(
            env.storage().persistent().get::<bool>(&guard_key).unwrap(),
            false
        );
    }
    ...
    fn test_is_migration_needed() {
        ...
        env.storage()
            .persistent()
            .set(&version_key, &(STORAGE_VERSION + 1));
...
pub fn initialize_contract_version(env: &Env) {
    if !env.storage().persistent().has(&CONTRACT_VERSION_KEY) {
