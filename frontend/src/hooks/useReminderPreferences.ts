...
export function useReminderPreferences() {
  ...
  useEffect(() => {
    const handlePreferencesChanged = (event: Event | CustomEvent<ReminderPreferences>) => {
      if ('detail' in event) {
    ...
    window.addEventListener('reminder-preferences-changed', handlePreferencesChanged);
    window.addEventListener('storage', handlePreferencesChanged);

    return () => {
      window.removeEventListener('reminder-preferences-changed', handlePreferencesChanged);
