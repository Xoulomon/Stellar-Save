      ...
      {isLoading
        ? [1, 2, 3].map((i) => (
            <Box key={i} sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
              ...
              <Skeleton variant="rect" width={50} height={22} rounded />
            </Box>
          ))
        : payouts.map((item) => (
            <Box key={item.id} sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
              <Avatar sx={{ bgcolor: item.status === 'upcoming' ? 'rgba(31,79,212,0.1)' : 'rgba(0,143,140,0.1)', color: item.status === 'upcoming' ? 'primary.main' : 'secondary.main', width: 40, height: 40 }}>
              ...
              <Box sx={{ flex: 1 }}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 0.5 }}>
                  ...
                  <Chip
                    label={item.status === 'upcoming' ? 'UPCOMING' : 'RECEIVED'}
                    size="small"
                    color={item.status === 'upcoming' ? 'warning' : 'success'}
                    variant="soft"
