            ...
            <Box sx={{ bgcolor: 'action.hover', borderRadius: 2, p: 2 }}>
              <Stack spacing={1}>
                {tx.from && (
                  <DetailRow
                    label="From"
                    value={formatAddress(tx.from, { prefixChars: 8, suffixChars: 6 })}
                    mono
                  />
                )}
                {tx.to && (
                  <DetailRow
                    label="To (contract)"
                    value={formatAddress(tx.to, { prefixChars: 8, suffixChars: 6 })}
                    mono
                  />
                )}
    ...
    <Stack spacing={3} alignItems="center" py={3}>
      ...
      <Stack spacing={1} alignItems="center">
        ...
        <Typography variant="body2" color="text.secondary" textAlign="center">
          Please approve the transaction in your Freighter wallet, then wait for network
          confirmation.
        </Typography>
      </Stack>
    </Stack>
      ...
      <Box sx={{ bgcolor: 'action.hover', borderRadius: 2, p: 2, width: '100%' }}>
        <Stack spacing={1}>
          <DetailRow
            ...
            value={formatAddress(txHash, { prefixChars: 10, suffixChars: 8 })}
            mono
          />
          {tx.amount !== undefined && <DetailRow label="Amount" value={formatAmount(tx.amount)} />}
        </Stack>
  ...
  return (
    ...
    >
      ...
      <DialogContent sx={{ pt: 2 }}>
        {step === 'review' && (
          <ReviewStep tx={transaction} onEdit={handleClose} onNext={() => setStep('confirm')} />
        )}
        {step === 'confirm' && (
          <ConfirmStep
            tx={transaction}
