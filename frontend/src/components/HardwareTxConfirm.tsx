...
export function HardwareTxConfirm({
  open,
  onClose,
  request,
  walletState,
  onSign,
}: HardwareTxConfirmProps) {
  ...
  return (
    <Dialog
      ...
      maxWidth="sm"
      fullWidth
      disableEscapeKeyDown={status === 'pending_device'}
    >
      <DialogTitle>
          ...
          {status === 'idle' && (
            <Stack spacing={2} sx={{ pt: 1 }}>
              <Alert severity="info" sx={{ fontSize: '0.8rem' }}>
                Review the transaction details below, then confirm on your{' '}
                {walletState.device?.name || 'hardware device'}.
              </Alert>
          ...
          {status === 'pending_device' && (
            <Stack spacing={2} alignItems="center" sx={{ py: 2 }}>
              ...
              <Typography variant="body2" color="text.secondary" textAlign="center">
                Please review and approve the transaction on your{' '}
                {walletState.device?.name || 'device'}.
                <br />
                The screen will update once confirmed.
              </Typography>
              {walletState.device?.type === 'ledger' && (
                <Alert severity="info" sx={{ fontSize: '0.8rem', width: '100%' }}>
                  On your Ledger: Navigate to the Stellar app, review the operation, press both
                  buttons to sign.
                </Alert>
              )}
