...
export function MemberCard({
  ...
  if (compact) {
    return (
      ...
      >
        <div
          className={`w-9 h-9 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0 ${avatarColor}`}
        >
        ...
        <span
          className={`text-xs font-semibold px-2 py-0.5 rounded-full flex-shrink-0 ${statusConfig.className}`}
        >
          {statusConfig.label}
        </span>
      </div>
    ...
    >
      <div className="relative bg-gradient-to-r from-indigo-50 to-purple-50 px-5 pt-5 pb-10">
        <div className="absolute top-4 right-4">
          <span
            className={`text-xs font-semibold px-2.5 py-1 rounded-full ${statusConfig.className}`}
          >
            {statusConfig.label}
          </span>
        </div>
      ...
      <div className="flex justify-center -mt-8 mb-3">
        <div
          className={`w-16 h-16 rounded-full border-4 border-white shadow flex items-center justify-center text-white text-xl font-bold ${avatarColor}`}
        >
          {getInitials(member.name, member.address)}
        </div>
      </div>
      ...
      <div className="px-5 pb-4">
        <div className="flex justify-between text-xs text-gray-400 mb-1">
          ...
          <span>
            {member.payoutPosition} / {member.totalMembers}
          </span>
        </div>
