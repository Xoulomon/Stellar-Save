...
function runMiddleware(
  middleware: ReturnType<typeof createRateLimiterMiddleware>,
  req: any,
  res: any
): boolean {
...
async function runTests() {
  ...
  // Sliding window strategy behavior
  {
    ...
    assert(
      first.res.headers['X-RateLimit-Remaining'] === '1',
      'sets X-RateLimit-Remaining after first hit'
    );
    ...
    assert(
      third.res.headers['Retry-After'] !== undefined,
      'blocked request sets Retry-After header'
    );
  ...
  // Per-IP for unauthenticated and per-user for authenticated users
  {
    ...
    assert(
      runMiddleware(limiter, unauth1.req, unauth1.res),
      'unauthenticated IP first request allowed'
    );

    nowMs = 10;
    const unauth2 = makeReqRes({ ip: '10.0.0.2' });
    assert(
      !runMiddleware(limiter, unauth2.req, unauth2.res),
      'unauthenticated IP second request blocked by IP limit'
    );
    ...
    assert(
      runMiddleware(limiter, auth1.req, auth1.res),
    ...
    assert(
      runMiddleware(limiter, auth2.req, auth2.res),
      'authenticated user second request allowed'
    );
    ...
    assert(
      !runMiddleware(limiter, auth3.req, auth3.res),
      'authenticated user third request blocked by user limit'
    );
  }
  ...
  // Authenticated bypass when request is authenticated but no user id is available
  {
    ...
    assert(
      runMiddleware(limiter, bypass1.req, bypass1.res),
      'authenticated request without user id bypasses rate limit (1)'
    );
    nowMs = 10;
    assert(
      runMiddleware(limiter, bypass2.req, bypass2.res),
      'authenticated request without user id bypasses rate limit (2)'
    );
