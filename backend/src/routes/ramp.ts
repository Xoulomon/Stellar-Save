...
export function createRampRouter(): Router {
  ...
  router.post('/deposit', jwtAuthMiddleware, rampProtection({ velocityCheck: true }), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    ...
    if (!anchorDomain || !assetCode || !stellarAccount) {
      return next(new AppError('MISSING_FIELDS', 'anchorDomain, assetCode, stellarAccount are required', 400));
    }
    const kyc = await getKycStatus(req.walletAddress!);
    if (kyc.status !== 'approved') {
      return next(new AppError('KYC_REQUIRED', 'KYC approval required to use fiat ramp', 403, { kycStatus: kyc.status }));
    }
    try {
      ...
      if (err instanceof CircuitBreakerOpenError || err?.code === 'CIRCUIT_OPEN') {
        return next(new AppError('RAMP_CIRCUIT_OPEN', 'Fiat ramp provider is currently unavailable (circuit open)', 503, err?.message));
      }
      return next(new AppError('DEPOSIT_INITIATION_FAILED', 'Failed to initiate deposit', 502, err?.message));
    }
  });

  // POST /api/ramp/withdraw
  router.post('/withdraw', jwtAuthMiddleware, rampProtection({ velocityCheck: true }), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    const { anchorDomain, assetCode, assetIssuer, amount, stellarAccount } = req.body as Record<string, string>;
    if (!anchorDomain || !assetCode || !stellarAccount) {
      return next(new AppError('MISSING_FIELDS', 'anchorDomain, assetCode, stellarAccount are required', 400));
    }
    ...
    try {
      ...
      return next(new AppError('WITHDRAW_INITIATION_FAILED', 'Failed to initiate withdraw', 502, err?.message));
    }
  });

  // GET /api/ramp/:id/status — sync and return latest status
  router.get('/:id/status', jwtAuthMiddleware, rampProtection({ velocityCheck: false }), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
  ...
  // GET /api/ramp/:id
  router.get('/:id', jwtAuthMiddleware, rampProtection({ velocityCheck: false }), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
